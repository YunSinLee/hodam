...
-- Keep both the picturebook paths and earlier v1 user/thread cover paths readable.
BEGIN;
  ...
  bucket_id = 'image' AND EXISTS (
    ...
    WHERE t.user_id = (SELECT auth.uid())
      AND (name = 'image_thread_id_' || t.id::text
        ...
        OR name ~ ('^' || t.user_id::text || '/thread_' || t.id::text || '/cover_[0-9]+\.png$'))
  )
)
WITH CHECK (
  bucket_id = 'image' AND EXISTS (
...
USING (
  bucket_id = 'profiles' AND (
    name ~ ('^profile_' || (SELECT auth.uid())::text || '_[a-z0-9-]+\.(jpg|png|webp|gif)$')
    OR name ~ ('^' || (SELECT auth.uid())::text || '/profile_[0-9]+\.[a-z0-9]+$')
  )
)
WITH CHECK (
  bucket_id = 'profiles' AND (
