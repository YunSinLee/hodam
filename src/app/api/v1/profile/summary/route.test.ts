...
describe("GET /api/v1/profile/summary", () => {
  ...
  it.each([
    ["https://cdn.example.com/avatar.png", null, false],
    [
      "profiles:profile_user-1_abcd-1234.png",
      "profile_user-1_abcd-1234.png",
      false,
    ],
    [
    ...
    "returns profile summary and resolves avatar %s (signing failure: %s)",
    async (reference, path, signingFailure) => {
      authenticateRequestMock.mockResolvedValue({
        accessToken: "token-1",
        userId: "user-1",
        email: "user@example.com",
      ...
      const messagesSelectMock = vi
        .fn()
        .mockReturnValue({ in: messagesInMock });

      const keywordsInMock = vi.fn().mockResolvedValue({
        data: [{ thread_id: 10, keyword: "용기" }],
        error: null,
      });
      const keywordsSelectMock = vi
        .fn()
        .mockReturnValue({ in: keywordsInMock });
      ...
      const createSignedUrlMock = vi.fn().mockResolvedValue({
        data: signingFailure
          ? null
          : { signedUrl: "https://cdn.example.com/signed/avatar.png" },
        error: signingFailure ? { message: "signing failed" } : null,
      });
      ...
      let expectedProfileUrl = reference;
      if (path) {
        expectedProfileUrl = signingFailure
          ? ""
          : "https://cdn.example.com/signed/avatar.png";
      }
